...
R3BFi4t::R3BFi4t(const TString& geoFile, const TGeoTranslation& trans, const TGeoRotation& rot)
    : R3BFi4t(geoFile, { trans, rot })
{
}

R3BFi4t::R3BFi4t(const TString& geoFile, const TGeoCombiTrans& combi)
    : R3BDetector("R3BFi4t", kFI4t, geoFile, combi)
...
void R3BFi4t::Initialize()
{
    FairDetector::Initialize();

    LOG(INFO) << "R3BFi4t: initialisation";
    LOG(DEBUG) << "R3BFi4t: Vol. (McId) " << gMC->VolId("FI4t1Log");
}

void R3BFi4t::SetSpecialPhysicsCuts()
{
    LOG(INFO) << "-I- R3BFi4t: Adding customized Physics cut ... ";
    ...
    if (gGeoManager)
    {
        ...
        if (pSi && 1 == 0)
        {
            ...
            LOG(INFO) << "-I- R3BFi4t: silicon Medium Id " << pSi->GetId() << " Energy Cut-Off : " << cutE << " GeV";
...
Bool_t R3BFi4t::ProcessHits(FairVolume* vol)
{
    // 2 Simple Det PLane
...
void R3BFi4t::BeginEvent() {}

// -----   Public method EndOfEvent   -----------------------------------------
void R3BFi4t::EndOfEvent()
{
    if (fVerboseLevel)
        Print();
...
TClonesArray* R3BFi4t::GetCollection(Int_t iColl) const
{
    if (iColl == 0)
...
// -----   Public method Print   ----------------------------------------------
void R3BFi4t::Print(Option_t* option) const
{
    Int_t nHits = fFi4Collection->GetEntriesFast();
    LOG(INFO) << "R3BFi4t: " << nHits << " points registered in this event";
}
// ----------------------------------------------------------------------------
...
void R3BFi4t::CopyClones(TClonesArray* cl1, TClonesArray* cl2, Int_t offset)
{
    Int_t nEntries = cl1->GetEntriesFast();
    LOG(INFO) << "R3BFi4t: " << nEntries << " entries to add";
    ...
    LOG(INFO) << "R3BFi4t: " << cl2->GetEntriesFast() << " merged entries";
}

// -----   Private method AddHit   --------------------------------------------
R3BFi4Point* R3BFi4t::AddHit(Int_t trackID,
                            Int_t detID,
                            Int_t plane,
                            TVector3 posIn,
                            TVector3 posOut,
                            ...
                            Double_t eLoss)
{
    ...
    if (fVerboseLevel > 1)
    {
        LOG(INFO) << "R3BFi4t: Adding Point at (" << posIn.X() << ", " << posIn.Y() << ", " << posIn.Z()
